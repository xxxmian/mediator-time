...
public class temp2 {
        ...
        int m1_pinDirection=1;
        int m1_pinSpeed=40;
        int m1_speed_value=40;
        Boolean m1_speed_reqRead = false;
        Boolean m1_speed_reqWrite = false;
        int m2_pinDirection = 1;
        int m2_pinSpeed = 40;
        int m2_speed_value = 40;
        ...
        public void output(int step){
            ...
            while(_step < step) {
                ...
                if (!((!(false) && !(m1_speed_reqRead)) || (!(false) && !(m2_speed_reqRead)) || (!(false) && !(c_act_reqWrite)) || (!(false) && c_act_reqWrite != m1_speed_reqWrite) || (!(false || c_act_reqWrite != m1_speed_reqWrite) && m1_speed_reqRead && m2_speed_reqRead != c_act_reqRead) || (!(false) && !(c_dist_reqWrite)) || (!(false || c_act_reqWrite != m1_speed_reqWrite || m1_speed_reqRead && m2_speed_reqRead != c_act_reqRead) && c_act_reqRead && c_act_reqWrite && !(false || !(c_dist_reqWrite)) && c_dist_reqRead || System.currentTimeMillis() - _ultraSonicDist_1_time1 >= 5 && !(false || !(m1_speed_reqRead)) && m1_speed_reqRead && m1_speed_reqWrite && !(false || !(m2_speed_reqRead)) && m2_speed_reqRead && m2_speed_reqWrite && !(false || !(c_act_reqWrite)) && c_act_reqRead))) {
                    System.out.println("deadlock!");
                    break;
                }
                Random r = new Random();
                pmark = r.nextInt(7);
                if ((pmark == 0) && (!(false) && !(m1_speed_reqRead))) {
                    m1_speed_reqRead = true;
                    _step += 1;
                }
                if ((pmark == 1) && (!(false) && !(m2_speed_reqRead))) {
                    m2_speed_reqRead = true;
                    _step += 1;
                }
                if ((pmark == 2) && (!(false) && !(c_act_reqWrite))) {
                    c_act_reqWrite = true;
                    _step += 1;
                }
                if ((pmark == 3) && (!(false) && c_act_reqWrite != m1_speed_reqWrite)) {
                    m1_speed_reqWrite = c_act_reqWrite;
                    m2_speed_reqWrite = c_act_reqWrite;
                    _step += 1;
                }
                if ((pmark == 4) && (!(false || c_act_reqWrite != m1_speed_reqWrite) && m1_speed_reqRead && m2_speed_reqRead != c_act_reqRead)) {
                    c_act_reqRead = m1_speed_reqRead & m2_speed_reqRead;
                    _step += 1;
                }
                if ((pmark == 5) && (!(false) && !(c_dist_reqWrite))) {
                    c_dist_reqWrite = true;
                    _step += 1;
                }
                if ((pmark == 6) && (!(false || c_act_reqWrite != m1_speed_reqWrite || m1_speed_reqRead && m2_speed_reqRead != c_act_reqRead) && c_act_reqRead && c_act_reqWrite && !(false || !(c_dist_reqWrite)) && c_dist_reqRead || System.currentTimeMillis() - _ultraSonicDist_1_time1 >= 5 && !(false || !(m1_speed_reqRead)) && m1_speed_reqRead && m1_speed_reqWrite && !(false || !(m2_speed_reqRead)) && m2_speed_reqRead && m2_speed_reqWrite && !(false || !(c_act_reqWrite)) && c_act_reqRead)) {
                    c_dist_reqWrite = false;
                ...
                if (((Object) c_act_reqWrite instanceof Integer) || ((Object) c_act_reqWrite instanceof Boolean)) {
                    ...
                    System.out.println(System.currentTimeMillis()-_ultraSonicDist_1_time1);
